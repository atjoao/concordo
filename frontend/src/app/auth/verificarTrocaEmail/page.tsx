...
function WaitingForSearchParams({ serverIp, sucesso, erro, setErro, setSucesso }: functionParams) {
    ...
    function sendVer() {
        ...
        fetch(serverIp + "/auth/verificarTrocaEmail?" + new URLSearchParams({ key: key, email: email }))
            .then(async (resp) => {
    ...
    return (
        <>
            <div className={styles.separator}>
                <p>Está prestes a trocar o email da conta {username}</p>
                <p>para o email: {email}</p>
            </div>
                    ...
                    <div>
                        ...
                        {sucesso && (
                            <div className={"sucesso " + formstyle.correct}>
                                <p>Troca de email concluida!</p>
                            </div>
                        )}
                    </div>
                    <div className={formstyle.formElement}>
                        <h1>Trocar email da conta</h1>
                        <Suspense>
